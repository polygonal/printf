...
class Main
{
	public static function main()
	{
		...
		if (!~/printf\/tools\/doc\//.match(cwd)) error('wrong working directory: $cwd');
		rmdir("./output");
		...
		{
			rmdir("./xml");
			var args = ["de.polygonal.Printf", "-swf", "tmp.swf", "--no-output", "-xml", "xml/swf.xml", "-cp", "./src"];
